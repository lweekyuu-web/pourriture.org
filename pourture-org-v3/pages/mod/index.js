...
export async function getServerSideProps({ req }) {
  ...
  const [pendingReports, lockedThreads, bannedUsers, pendingActions] = await Promise.all([
  ...
  return { props: { stats: { pendingReports, lockedThreads, bannedUsers, pendingActions } } };
}
...
Banned users: ${stats.bannedUsers}`}</pre>
      ...
      {stats.pendingReports > 0 && (
        <div className="notification">[{stats.pendingReports} NEW REPORT{stats.pendingReports > 1 ? 'S' : ''}]</div>
      )}
    </div>
  );
}

...
export default function AdminBoards({ boards: initialBoards }) {
  ...
  const [editForm, setEditForm] = useState({ name: '', description: '', rules: '', status: 'public' });
  ...
  async function updateBoard(e) {
    ...
    if (res.ok) {
      setBoards(boards.map((b) => b.id === editing.id ? { ...b, ...editForm } : b));
      setEditing(null);
    }
  }
  ...
  function startEdit(board) {
    setEditing(board);
    setEditForm({ name: board.name, description: board.description, rules: board.rules || '', status: board.status });
  }

    ...
    <>
      ...
      <div className={styles.container}>
        ...
        <div className={styles.filterBar}>
          {['PENDING', 'REVIEWED', 'ALL'].map((f) => <button key={f} className={`${styles.filterBtn} ${filter === f ? styles.active : ''}`} onClick={() => { setFilter(f); setCurrentIndex(0); }}>{f} ({reviews.filter((r) => f === 'ALL' || (f === 'PENDING' ? !r.reviewedAt : !!r.reviewedAt)).length})</button>)}
        </div>
          ...
          <div className={styles.reviewQueue}>
            <div className={styles.reviewCard}>
              <div className={styles.header}>
                <div className={styles.status}><span className={styles.badge}>{current.decision}</span><span className={styles.confidence}>{(current.confidence * 100).toFixed(0)}%</span></div>
                <small>Target: {current.targetType} #{current.targetId} · User {current.userId}</small>
              </div>

              <div className={styles.preview}>
                {current.targetType === 'avatar' && current.targetUrl ? <img src={current.targetUrl} alt="Avatar under review" /> : <div className={styles.textPreview}><p>{current.targetContent || 'No preview available.'}</p></div>}
              </div>

              <div className={styles.details}>
                <div className={styles.reasons}><strong>AI reasons:</strong><ul>{(current.reasons || []).map((reason, i) => <li key={i}>{reason}</li>)}</ul></div>
                {current.flaggedKeywords?.length > 0 && <div className={styles.keywords}><strong>Flagged:</strong>{current.flaggedKeywords.map((kw, i) => <span key={i} className={styles.keyword}>{kw}</span>)}</div>}
              </div>
              ...
              <div className={styles.navigation}>
                ...
                <span>{currentIndex + 1} / {filteredReviews.length}</span>
                <button onClick={() => setCurrentIndex((i) => Math.min(filteredReviews.length - 1, i + 1))} disabled={currentIndex >= filteredReviews.length - 1}>Next →</button>
              </div>
            </div>
          </div>
        )}
...
export async function getServerSideProps({ req }) {
  try {
    ...
    return { props: { isAuthorized: true, initialReviews: reviews } };
  } catch (error) {

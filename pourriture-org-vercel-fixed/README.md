...
5. Une fois déployé, va sur `https://TON-PROJET.vercel.app/admin/login`, connecte-toi avec `ADMIN_PASSWORD`, puis clique sur le lien "Peupler la base avec les boards/threads de démo" sur le dashboard.
6. Pour utiliser ton domaine `pourriture.org` : dans Vercel → Project → Settings → Domains → ajoute `pourriture.org`. Vercel te donne un enregistrement DNS à configurer chez ton registrar de domaine (là où tu as acheté le domaine) — remplace l'ancien enregistrement qui pointait vers GitHub Pages.
...
## Vercel / production
This repository is structured as a Next.js application at the repository root so Vercel can detect Next.js automatically. Production domain: `https://pourriture.org`. Local development may still use `http://localhost:3000`.

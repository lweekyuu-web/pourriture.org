...
export async function getServerSideProps({ req }) {
  ...
  const [topWidgets, bottomWidgets, settings, totalThreads, totalUsers] = await Promise.all([
    getWidgetsForSlot('home_top'), getWidgetsForSlot('home_bottom'), getAllSettings(),
    prisma.thread.count(), prisma.user.count(),
  ]);
...
function fmt(dateStr) {
  ...
  return d.toLocaleDateString('en-US', { month:'2-digit', day:'2-digit', year:'2-digit', timeZone:'utc' }) + ' ' + d.toLocaleTimeString('en-GB', { timeZone:'utc' });
}
...
export default function Home({ boards, topWidgets, bottomWidgets, settings, isAdmin, totalThreads, totalUsers }) {
  ...
  async function saveMotd() {
    const res = await fetch('/api/admin/settings', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ motd:motdValue }) });
    if (res.ok) { setSaved(true); setTimeout(()=>setSaved(false),1500); setEditingMotd(false); window.location.reload(); }
  }
  ...
  return <>
    ...
    <div className="container">
      ...
      <div className="home-tools">
        <div className="home-stats"><b>{boards.length}</b> boards · <b>{totalThreads}</b> threads · <b>{totalUsers}</b> users</div>
        <form className="board-jump" onSubmit={jump}><label htmlFor="jumpBoard">Jump to:</label><select id="jumpBoard" value={jumpBoard} onChange={e=>setJumpBoard(e.target.value)}><option value="">select a board</option>{boards.map(b=><option key={b.id} value={b.id.replace(/\//g,'')}>{b.id} — {b.name}</option>)}</select><button type="submit" disabled={!jumpBoard}>Go</button></form>
      </div>

      <WidgetSlot widgets={topWidgets} />
      <div className="section-heading"><b>PUBLIC BOARDS</b><span>last activity shown in UTC</span></div>
      {boards.map(b => <div className="board-row" key={b.id}><div><span className="bid">[{b.id.replace(/\//g,'')}]</span> <Link href={`/${b.id.replace(/\//g,'')}`}><a>{b.id} {b.name}</a></Link></div><div className="desc">{b.description}</div><div className="meta"><b>{b.threadCount}</b> threads — Last activity: {fmt(b.lastActivity)}</div></div>)}

...
function isAllowedImage(url) {
  ...
  try { const parsed = new URL(url); return parsed.protocol === 'https:' && parsed.hostname.length <= 253; } catch { return false; }
}
function isAllowedGif(url) {
  if (!url) return true;
  try { const parsed = new URL(url); return parsed.protocol === 'https:' && /(^|\.)giphy\.com$/i.test(parsed.hostname); } catch { return false; }
}
async function moderateInput(message, gifUrl, imageUrl) {
  ...
  try {
    ...
    const data = await response.json(); const results = data.results || [];
    let textFlagged = false, gifFlagged = false, imageFlagged = false;
    input.forEach((item, index) => { if (!results[index]?.flagged) return; if (item.type === 'text') textFlagged = true; else if (item.image_url?.url === gifUrl) gifFlagged = true; else if (item.image_url?.url === imageUrl) imageFlagged = true; });
    return { textFlagged, gifFlagged, imageFlagged, aiAvailable: true };
  } catch (_) { return { textFlagged: false, gifFlagged: false, imageFlagged: false, aiAvailable: false }; }
}
...
export default async function handler(req, res) {
  ...
  const author = uid ? await prisma.user.findUnique({ where: { id: uid }, select: { displayName: true, role: true } }) : null;
  const isStaff = author?.role === 'moderator' || author?.role === 'administrator';

  // Moderators/admins are trusted posters and must never be sent to their own review queue.
  if (!isStaff && await isBanned(uid)) return res.status(403).json({ error: 'This identity has been banned from posting.' });
  if (!isStaff && await isFlooding(uid)) return res.status(429).json({ error: 'You are posting too fast. Please slow down.' });
  const cleanMessage = String(message || '').trim().slice(0, 5000);
  if (!isStaff && cleanMessage && containsBannedContent(cleanMessage)) return res.status(400).json({ error: 'Your message was rejected by the content filter.' });
  ...
  const ai = isStaff ? { textFlagged: false, gifFlagged: false, imageFlagged: false, aiAvailable: true } : await moderateInput(cleanMessage, gifUrl, imageUrl);
  const mediaNeedsReview = !isStaff && !ai.aiAvailable && Boolean(gifUrl || imageUrl);
  const heldForReview = !isStaff && (ai.textFlagged || ai.gifFlagged || ai.imageFlagged || mediaNeedsReview);
  const reviewReason = ai.textFlagged || ai.gifFlagged || ai.imageFlagged ? 'AI moderation flagged this submission; moderator review required.' : (mediaNeedsReview ? 'Automatic image moderation is temporarily unavailable; this media will be reviewed.' : null);

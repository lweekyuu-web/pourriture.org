...
export default function ThreadPage({ thread, boardSlug, topWidgets, bottomWidgets, isMod }) {
  ...
  async function submitReply(e) {
    e.preventDefault();
    const res = await fetch('/api/posts', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ threadId: thread.id, message, gifUrl, imageUrl, replyToId: replyTo }) });
    const data = await res.json().catch(() => ({}));
    if (res.ok) { if (!data.review) setPosts([...posts, data.post]); setMessage(''); setGifUrl(''); setImageUrl(''); setReplyTo(null); if (data.review) alert(data.reviewReason || 'Your submission is waiting for moderator review.'); }
  ...
  return <main className="container">
    ...
    {posts.map((p, idx) => { const author = p.author, avatarVisible = author?.avatarUrl && author.avatarStatus === 'approved', badge = author?.badge || '', replyTarget = p.replyToId ? posts.find((target) => target.id === p.replyToId) : null, isReview = p.status === 'review'; return <div className={`post ${collapsed[p.id] ? 'post-collapsed' : ''}`} id={`post-${p.id}`} key={p.id}>
      <div className="post-head">{avatarVisible && <img className="post-avatar" style={avatarStyle} src={author.avatarUrl} alt="" loading="lazy" />}<a className="name post-author-name" style={nameStyle(author)} href={author ? `/user/${author.anonId}` : '#'} onClick={!author ? (e) => e.preventDefault() : undefined}>{p.displayName || 'Anonymous'}</a>{author?.anonId && <span className="anon-post-id"> ({author.anonId})</span>}{badge && <span className="post-badge" style={badgeStyle}>[{badge}]</span>}{idx === 0 && <span className="badge op-badge">[OP]</span>}<span className="date">{fmt(p.createdAt)}</span> <Link className="postnum" href={`/post/${encodeURIComponent(p.postNumber)}`}>No.{p.postNumber}</Link>{p.editedAt && <span className="muted"> (edited {fmt(p.editedAt)})</span>}<a href="#" className="collapse-link" onClick={(e) => { e.preventDefault(); setCollapsed({ ...collapsed, [p.id]: !collapsed[p.id] }); }}>[{collapsed[p.id] ? '+' : '−'}]</a></div>
      {!collapsed[p.id] && <>{replyTarget && <div className="quote"><a href="#" onClick={(e) => { e.preventDefault(); jumpTo(replyTarget.id); }}>&gt;&gt;{replyTarget.postNumber}</a></div>}{isReview ? <div className="review-note">⚠ This comment is being reviewed by a moderator.</div> : <>{p.content && <div className="content">{p.hidden ? '[post removed by moderator]' : p.content}</div>}{p.imageUrl && !p.hidden && <div className="post-gif"><img src={p.imageUrl} alt="Attached image" loading="lazy" style={mediaStyle}/></div>}{p.gifUrl && !p.hidden && <div className="post-gif"><img src={p.gifUrl} alt="GIF" loading="lazy" style={mediaStyle}/></div>}</>}
